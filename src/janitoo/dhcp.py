...
class JNTNetwork(object):
    ...
    def start(self):
        ...
        if self.fsm_network is None:
            self.fsm_network = Machine(model=self, states=self.fsm_network_states, initial='STOPPED')
            self.fsm_network.add_ordered_transitions()
            #~ self.fsm_network.add_transition('fsm_network_start', 'STOPPED', 'BROADCAST_START', conditions=['is_primary'])
            self.fsm_network.add_transition('fsm_network_start', 'STOPPED', 'BROADCAST_NODES', conditions=['fsm_is_primary'])
            self.fsm_network.add_transition('fsm_network_start', 'STOPPED', 'RESOLV', conditions=['fsm_is_secondary'])
            #~ self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_START', 'BROADCAST_NODES')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_NODES', 'BROADCAST_SYSTEMS')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_SYSTEMS', 'BROADCAST_COMMANDS')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_COMMANDS', 'BROADCAST_CONFIGS')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_CONFIGS', 'BROADCAST_BASICS')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_BASICS', 'BROADCAST_USERS')
            #~ self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_CONFIGS', 'BROADCAST_STOP')
            self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_USERS', 'HEARTBEAT_DISCOVER')
            #~ self.fsm_network.add_transition('fsm_network_next', 'BROADCAST_STOP', 'HEARTBEAT_DISCOVER')
            self.fsm_network.add_transition('fsm_network_next', 'HEARTBEAT_DISCOVER', 'DISPATCH', conditions=['fsm_do_heartbeat_dispatch'])
            self.fsm_network.add_transition('fsm_network_next', 'HEARTBEAT_DISCOVER', 'STARTED')
            self.fsm_network.add_transition('fsm_network_next', 'DISPATCH', 'STARTED')
            self.fsm_network.add_transition('fsm_network_next', 'RESOLV', 'HEARTBEAT')
            self.fsm_network.add_transition('fsm_network_next', 'HEARTBEAT', 'STARTED')
            self.fsm_network.add_transition('fsm_network_fail', 'STARTED', 'BROADCAST_NODES', before = ['set_failed'])
            self.fsm_network.add_transition('fsm_network_fail', 'RESOLV', 'BROADCAST_NODES', before = ['set_failed'])
            self.fsm_network.add_transition('fsm_network_recover', 'STARTED', 'RESOLV', before = ['stop_heartbeat_discover', 'stop_dispatch_heartbeat', 'unset_failed'])
            self.fsm_network.add_transition('fsm_network_stop', '*', 'STOPPED', after = ['delete_fsm'])
            self.state = 'STOPPED'
    ...
    def loop(self, mqttc, stopevent):
        ...
        if len(to_polls)>0:
            logger.debug('Found polls in timeout : %s', to_polls)
