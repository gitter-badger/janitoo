...
class TestArgParser(TestJanitoo):
    ...
    def test_050_args_to_dict(self):
        with mock.patch('sys.argv', [self.prog, 'start', '--conf_file=tests/data/test_runner_conf_complete.conf']):
            ...
            self.assertEqual(args['log_dir'], '/var/log/janitoo')
            self.assertEqual(args['home_dir'], '/home/janitoo')
            self.assertEqual(args['pid_dir'], '/var/run/janitoo')
            self.assertEqual(args['conf_dir'], '/etc/janitoo')
